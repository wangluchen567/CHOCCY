...
class DTLZ3(PROBLEM):
    ...
    def _cal_objs(self, X):
        D = self.num_dec
        M = self.num_obj
        g = 100 * (D - M + 1 + np.sum((X[:, M - 1:] - 0.5) ** 2 - np.cos(20 * np.pi * (X[:, M - 1:] - 0.5)),
                                      axis=1))
        objs = (np.tile(1 + g, (M, 1)).T *
                  np.fliplr(np.cumprod(np.hstack((np.ones((X.shape[0], 1), dtype=float),
                                                  np.cos(X[:, :M - 1] * np.pi / 2))), axis=1)) *
                  np.hstack((np.ones((X.shape[0], 1), dtype=float),
