...
## 简介
CHOCCY是一个基于 NumPy 构建的启发式优化求解器，支持对`实数`、`整数`、`序列`、`(固定)标签`以及`混合`类型问题的优化求解。<br>
...
class Sphere(PROBLEM):
    def __init__(self, num_dec=30, lower=-100, upper=100):
        # 调用父类PROBLEM的构造方法来初始化父类的属性，子类Sphere会继承父类PROBLEM的属性和方法。
...
此外，本项目还支持在不绘制任何图像的情况下仅输出日志的功能，并且支持将日志保存到文件。这一特性特别适用于那些无法进行绘图的环境，例如基于 Docker 的部署环境。具体使用方法是将 `show_mode` 设置为 `View.LOG`，即可将日志输出到控制台。如果需要将日志保存到文件，只需在算法优化前启用文件保存功能。日志文件默认保存在 `Outputs/Logs/` 文件夹中，文件名默认为 `[算法名称].log`。
...
本项目支持对优化结果与历史信息进行保存，方便后续分析与对比。保存路径默认为 `Outputs/` 文件夹，具体路径会根据问题类型进行区分：单目标问题的保存路径为 `Outputs/Single/`；多目标问题的保存路径为 `Outputs/Multi/`
...
除了保存功能之外，本项目还支持对已保存的优化结果进行读取。默认的读取路径为 `Outputs/` 文件夹，但在读取时需要明确指定要读取的具体文件夹路径。以下是读取优化结果的示例代码：

```python
# 读取 Outputs 文件夹下的 Single/DE_solve_Ackley_best 文件夹中的最优解相关信息
algorithm.load_best("Single/DE_solve_Ackley_best")

# 读取 Outputs 文件夹下的 Single/DE_solve_Ackley_pop 文件夹中的种群相关信息
algorithm.load_pop("Single/DE_solve_Ackley_pop")

# 读取 Outputs 文件夹下的 Single/DE_solve_Ackley_history 文件夹中的种群历史相关信息
algorithm.load_history("Single/DE_solve_Ackley_history")
```

请注意，读取函数的名称必须与要读取的文件夹名称一致，否则可能会导致读取失败或报错。
...
与前面介绍的问题定义方式类似，带约束的问题同样需要明确其关键信息。以一个简单的带约束问题为例，假设要求解的问题是 $ f(\vec{x}) = \vec{x}^T \vec{x} $（即向量的平方和），其约束条件为：$ x_1 + x_2 + \dots + x_n > 1 $（即向量中所有元素的和大于1），并且 $ x_1 > 0, x_2 > 0, \dots, x_n > 0 $（即向量中的每个元素均大于0）。我们可以确定该问题的关键信息如下：

1. **问题的类型**：该问题是单纯的 `实数` 问题。
2. **决策变量的个数**：不确定，但可以指定，默认为 30。
3. **优化目标的个数**：只有单个目标。
4. **决策变量下界**：确定，每个决策变量的最小值为 0。
5. **决策变量上界**：不确定，但为了缩小范围，均指定为 100。
...
在确定好问题的关键信息之后，我们就可以根据这些信息初始化问题类了。所有的问题类都必须继承自 `Problems` 模块下的 `PROBLEM` 父类，以便于后续的预处理和算法调用。
...
**请注意**：约束条件的计算函数需要转换为单侧小于0的不等式形式，以上面的约束$ x_1 + x_2 + \dots + x_n > 1 $为例，该约束需要转换为 $ g(\vec{x}) = 1 - (x_1 + x_2 + \dots + x_n) < 0 $。这是因为算法在判断一个解是否满足约束时，若约束函数$g(\vec{x})$得到的约束值大于0，则说明该解不满足约束，且不满足约束的程度即为计算得到的约束值。算法会尽量减少约束值，以使解尽可能满足约束条件。

另外，实现问题的约束函数时，与实现目标函数时类似，可以通过覆写 `_cal_con` 或 `_cal_cons` 中的任意一个方法来完成约束函数的实现，两者的区别在于：`_cal_con` 用于针对单个解计算约束值，而 `_cal_cons` 则用于批量计算多个解的约束值。

以下是一个覆写 `_cal_con` 方法的示例：
...
如果仅覆写了 `_cal_con` 方法， 父类会自动通过 `for` 循环逐次调用 来计算多个解的约束值。虽然这种实现方式在逻辑上没有任何问题，但运行效率可能较低。为了突破 Python 中 `for` 循环的效率瓶颈，推荐覆写 `_cal_cons` 方法，并利用 `numpy` 的矩阵操作来计算目标值，这样计算效率会更高。

此外，在实现带约束问题时，请注意以下几点：
1. 对于复杂约束问题，或者因约束导致可行解空间较窄的情况，算法可能无法找到完全满足约束的解。此时，算法返回的最优解将是尽可能满足约束的解，即约束值尽可能小的解。
2. 启发式算法本身并不擅长处理约束问题，本项目采用类似约束松弛的方法来处理约束。因此，可能需要自行设计一些处理约束的技巧。在必要时，建议通过一些技巧将问题转换为无约束问题，或者将约束隐含在问题中，以提高启发式算法的求解效果。

### 算法比较器的使用

算法比较器是用于比较不同算法在同一个问题上的表现，这有助于研究算法的性能差异。为了方便实时比较算法的效果，比较器支持在每一步迭代中将所有算法绘制到同一图像中。

调用比较器非常简单：首先确定要优化的问题，然后将所有算法放入同一个字典或者列表中，若使用列表，算法不可以重复使用；若使用字典，字典的键值建议为算法名称，以便于绘图和区分。需要注意的一个重要参数是 `same_init`，它的作用是确保所有算法尽可能地使用一致的初始化条件（‘尽可能地保持一致’是考虑到某些算法的特殊性，例如只包含单个个体的算法，只能单个个体一致）。
...
另外，比较器还提供了一个参数`show_colors`，可以通过该参数来指定绘图时使用的颜色。

请注意，选择比较的所有算法必须都能求解指定问题，否则很可能会导致报错。

### 算法评估器的使用

算法评估器是用于比较不同算法在不同问题（或同一问题）上多次运行表现的工具。由于本项目中的大多数算法具有随机性，因此在比较算法的优劣时，可以通过多次运行并结合统计检验来得出更可靠的结论。

调用算法评估器非常简单。首先，确定一个或多个问题，并将这些问题放入问题字典或列表中。接着，选择要评估的算法，并将它们放入算法字典或列表中。需要注意的是，如果使用列表来指定问题或算法，问题和算法的名称不能重复，因为评估器会将列表自动转换为字典，而字典的键（key）为类的名称。如果直接使用字典来指定问题和算法，建议将字典的键值设置为问题或算法的名称，这样可以更方便地在打印或绘制评估结果时进行标识。

以下是一个示例，展示了多种算法在多个 SOP 问题上的表现：
...
另外，评估器还提供了一个参数`show_colors`，可以通过该参数来指定绘图时使用的颜色。

请注意，选择比较的所有算法必须都能求解所有问题，否则很可能会导致报错。

## 高级功能

### 混合类型问题的定义与实现

在 [问题定义与实现](#问题定义与实现) 中已经详细提到 `problem_type` 参数还可以通过输入 `np.array` 数组来指定每一位决策变量的类型，
...
- **问题 3**：在使用Evaluator对算法进行评估时，绘制的小提琴图上下界为什么会不准确？
  ...
  - **解决方案**：若想让小提琴图绘制准确，在函数`plot_violin()`中加入参数`accurate=True`即可
