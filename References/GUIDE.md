...
多目标算法用于求解多目标问题，旨在同时优化多个目标函数，常见的效果较好的且通用性强的多目标算法一般是元启发式方法，比如多目标进化算法(遗传算法)，本项目仅支持此类多目标算法，具体支持的算法清单可参见[实现清单](./IMPLES.md)
...
在使用优化器求解问题之前需要先实现需要求解的问题，在实现问题之前，需要先确定问题的一些关键信息：
...
以一个简单的问题为例，假设要求解的问题是$f(\vec{x})=\vec{x}^T\vec{x}$（即向量的平方和），那么我们可以确定该问题的信息：

1. 问题的类型是单纯的`实数`问题
2. 决策变量个数不确定，是可以指定的
3. 只有单个目标
4. 决策变量下界不确定，但为了缩小范围，均指定为-10
5. 决策变量上界不确定，但为了缩小范围，均指定为10

在确定问题的信息时需要注意，由于给出的例子很特殊，最终结果是求和，所以决策变量个数可以不确定，但一般来说，具体问题的决策变量个数都是确定的。对于问题的决策变量上下界，建议在确定问题信息时大概估算一个数值范围，这个范围尽可能包含函数的最优解。

在确定好信息之后，我们就可以根据这个信息初始化问题类了，所有的问题类必须继承 Problems 下的 PROBLEM 父类，以方便预处理和算法调用：
...
其中对于决策变量上下界的指定，有一个小细节要注意，在给定上下界时，可以给定单独的数值，也可以给定`np.ndarray`数组。在给定数值时，表示所有的变量均是该下界/上界；给定数组时，数组大小必须与决策变量个数匹配，并且可以指定每一位的决策变量范围。

在初始化问题之后，必须实现问题的目标函数，可以覆写`_cal_obj` 或 `_cal_objs` 两种中的其中一种以实现目标函数，两者的区别是`_cal_obj`是对单个解求目标值，`_cal_objs` 则是对多个解进行求目标值，如果只覆写了`_cal_obj`函数，PROBLEM父类会自动使用已有的`_cal_objs` 函数会使用`for`循环对多个解进行一个一个的计算目标值（参见源码）。为了破除`Python`中`for`循环的效率瓶颈，建议对`_cal_objs` 进行覆写并使用`Numpy`的矩阵操作计算目标值，效率更高。这里直接使用矩阵操作实现例子的目标值计算：
...
另外需要注意的一点是，输入的`x`变量一定是二维矩阵，即使是决策变量个数为`1`的问题，输入形状也是`(n, 1)`，`n`是解的个数
