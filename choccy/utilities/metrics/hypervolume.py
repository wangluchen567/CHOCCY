...
def calc_hv(objs: np.ndarray, optimums: Optional[Union[np.ndarray, list]] = None) -> float:
    ...
    # 获取目标值维度信息
    objs_size, objs_dim = objs.shape
    # 如果未提供理论最优目标值，则参考点向量全置1
    if optimums is None:
        refer_array = np.ones(objs_dim)
    else:
        refer_array = np.asarray(optimums)
    # 根据参考点向量规范化目标值
    ...
    if objs_norm.size == 0:
        ...
        # 计算精确的HV值
        score = calc_hv_accurate(objs_norm, ref_point)
    else:
        # 通过蒙特卡罗方法估计HV值
        score = calc_hv_estimated(objs_norm, ref_point)
    return score


def _calc_hv_estimated(objs_norm: np.ndarray, ref_point: np.ndarray, sample_num: int = 1e6) -> float:
    """
    使用蒙特卡罗方法估计HV值（使用numpy计算）
...
def _calc_hv_accurate(objs_norm: np.ndarray, ref_point: np.ndarray) -> float:
    """
    计算精确的HV值（使用numpy计算）
...
try:
    ...
    def array_equal(arr1: np.ndarray, arr2: np.ndarray, equal_nan: bool = False) -> bool:
        """
        检查两个数组是否具有相同的形状和元素。
        :param arr1: 第一个输入数组
        :param arr2: 第二个输入数组
    ...
    @jit(nopython=True, cache=True)
    def _head_jit(pl: np.ndarray) -> np.ndarray:
        if pl.shape[0] == 0:
            return np.empty(0)
        return pl[0]
    ...
    @jit(nopython=True, cache=True)
    def _tail_jit(pl: np.ndarray) -> np.ndarray:
        if pl.shape[0] <= 1:
    ...
    @jit(nopython=True, cache=True)
    def _add_jit(list_: List, s: List) -> List:
        num_s = len(s)
        found = False
    ...
    @jit(nopython=True, cache=True)
    def _insert_jit(p: np.ndarray, k: int, pl: np.ndarray) -> np.ndarray:
        m = p.shape[0]
    ...
    @jit(nopython=True, cache=True)
    def _slice_jit(pl: np.ndarray, k: int, ref_point: np.ndarray) -> List:
        s = List()
        if pl.shape[0] == 0:
            s.append((1.0, pl))
            return s
    ...
    @jit(nopython=True, cache=True)
    def _calc_hv_accurate_jit(objs_norm: np.ndarray, ref_point: np.ndarray) -> float:
        pl = objs_norm
        s = List()
    ...
    def _calc_hv_estimated_jit(objs_norm: np.ndarray, ref_point: np.ndarray, sample_num: int = 1e6) -> float:
        """
        使用蒙特卡罗方法估计HV值(使用numba多核并行加速)
    ...
    def calc_hv_accurate(objs_norm: np.ndarray, ref_point: np.ndarray) -> float:
        """
        精确计算HV值(默认使用numba加速)
        ...
        if objs_norm.size == 0:
            return 0.0
        return _calc_hv_accurate_jit(objs_norm, ref_point)


    def calc_hv_estimated(objs_norm: np.ndarray, ref_point: np.ndarray, sample_num: int = 1e6) -> float:
        """
        使用蒙特卡罗方法估计HV值(默认使用numba多核并行加速)
        ...
        :return:  HV分数值
        """
        return _calc_hv_estimated_jit(objs_norm, ref_point, sample_num)
    ...
    calc_hv_accurate = _calc_hv_accurate
    calc_hv_estimated = _calc_hv_estimated
