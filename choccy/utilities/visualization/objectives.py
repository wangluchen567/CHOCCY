...
def plot_objectives(objs: np.ndarray,
    ...
    if n_objs == 1:
        ...
        if pareto_front is not None:
            frame.add_line(pareto_front[:, 0], pareto_front[:, 1], marker="", c="gray", zorder=0)
        elif optimums is not None and isinstance(optimums, np.ndarray) and optimums.ndim == 2:
            frame.add_scatter(optimums[:, 0], optimums[:, 1], c="gray", s=6, alpha=0.6, zorder=0)
        ...
        if pareto_front is not None:
            if isinstance(pareto_front, list) and len(pareto_front) >= 3:  # 使用规则矩形网格绘制
                frame.add_wireframe(pareto_front[0], pareto_front[1], pareto_front[2],
                                    rstride=1, cstride=1, color='silver', linewidth=0.8, zorder=0)
                ...
                if xs is not None and ys is not None and zs is not None:
                    frame.add_trisurf(xs, ys, zs, edgecolor='gray', color=(1, 1, 1, 0), linewidth=0.16, zorder=0)
        elif optimums is not None and isinstance(optimums, np.ndarray) and optimums.ndim == 2:
            frame.add_scatter(optimums[:, 0], optimums[:, 1], optimums[:, 2], c="gray", s=3, alpha=0.3, zorder=0)
        # 设置三维图像角度(仰角方位角)
