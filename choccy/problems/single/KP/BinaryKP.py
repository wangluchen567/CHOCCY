...
class BinaryKP(Problem):
    def __init__(self,
        ...
        """
        super().__init__(self.BIN, n_vars, n_objs=1, l_bounds=0, u_bounds=1)
        # 设置随机种子
        ...
        if weights is not None or profits is not None or capacity is not None:
            ...
            # 若所有参数均为None，则随机生成
            self.weights = rng.integers(10, 100, size=self.n_vars)
            self.profits = rng.integers(10, 100, size=self.n_vars)
    ...
    def calc_objs_mat(self, xs):
        assert self.profits is not None
        objs = np.sum(self.profits) - xs.dot(self.profits)
        return objs
