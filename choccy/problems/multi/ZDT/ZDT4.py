...
class ZDT4(Problem):
    def __init__(self, n_vars: int = 10):
        ...
        :param n_vars: 决策变量个数
        """
        l_bounds = np.zeros(n_vars) - 5
        l_bounds[0] = 0
        u_bounds = np.zeros(n_vars) + 5
        u_bounds[0] = 1
        super().__init__(self.REAL, n_vars, n_objs=2, l_bounds=l_bounds, u_bounds=u_bounds)

    def calc_objs_mat(self, xs: np.ndarray):
        g = np.sum(xs[:, 1:] ** 2 - 10 * np.cos(4 * np.pi * xs[:, 1:]), axis=1)
        g = g + 1 + 10 * (self.n_vars - 1)
        f1 = xs[:, 0]
        f2 = g * (1 - np.sqrt(f1 / g))
